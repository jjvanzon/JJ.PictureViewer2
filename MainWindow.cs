...
namespace PictureViewer2
{
	...
	public partial class MainWindow : Form
	{
		...
		private void ShowPicture(string path)
		{
			try
			{
				// Try prevent keeping image file in use, by copyijng it.
				using (Image imageFromFile = new Bitmap(path))
				{
					Image copiedImage = new Bitmap(imageFromFile);
		...
		private void ProcessKey(char KeyChar)
		{
			...
			if (category != null)
			{
				...
				catch (IOException ex ) { MessageBox.Show("File access error. File may already exist.", Program.ApplicationName); }
				catch (BadPathFormatApplicationException ex) { MessageBox.Show("Bad folder format. Check the category configuration.", Program.ApplicationName); }
